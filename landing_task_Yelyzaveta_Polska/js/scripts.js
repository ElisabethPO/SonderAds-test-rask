...
(() => {
  ...
  const closeAllModals = () => {
    document
      .querySelectorAll(SELECTORS.visibleModal)
  ...
  document.querySelectorAll(SELECTORS.newsletter).forEach((newsletter) => {
...
if (ll__forms.length) {
  ll__forms.forEach((form) => {
    form.addEventListener("submit", async (e) => {
      ...
      const appendUrlParameters =
        form.dataset.llFormAppendParameters === "true";
      const popupDuration = Number(form.dataset.llFormPopupDuration) || 2000;
      const trustedFormCertUrl =
        document.querySelector('[name="xxTrustedFormCertUrl"]')?.value || "";
      ...
      if (window.RECAPTCHA_V3_SITE_KEY) {
        recaptchaScore = await new Promise((resolve) =>
          grecaptcha.ready(() =>
            grecaptcha
              .execute(window.RECAPTCHA_V3_SITE_KEY, { action: "submit" })
              .then(resolve),
          ),
        );
      }
      ...
      fields.forEach((field) => {
        ...
        if (inputWrapper === "tel") {
          ...
          if (isIntlInput) {
            ...
            data.value =
              `+${countryCode} ${telInput.value.replace(`+${countryCode}`, "")}`
                .replace(/[-\s()]/g, "")
                .replace("+Nocountryselected", "");
            telInput.value = "";
          }
        }
      ...
      if (clickFlareClickId)
        payload.data.push({
          ...
          value: clickFlareClickId,
          label: "ClickFlare Click Id",
        });
      if (trustedFormCertUrl)
        payload.data.push({
          key: Math.random().toString(36).substring(2),
          value: trustedFormCertUrl,
          label: "Trusted Form Cert Url",
        });
      if (jornayaLeadId)
        payload.data.push({
          key: Math.random().toString(36).substring(2),
          ...
          setTimeout(
            () => {
              if (appendUrlParameters) {
                ...
                window.open(
                  newUrl,
                  form.getAttribute("target") === "_blank" ? "_blank" : "_self",
                );
              } else {
                window.open(
                  redirectUrl,
...
function ll_run_event(el, href, target, type, event = "click") {
  ...
  el.addEventListener(event, (e) => {
    ...
    if (type === "redirect") {
      ...
      if (el.dataset.llUrlQueryParams)
        addUrlQueryParams(parseHref(href), target);
    ...
    llPopups.forEach((popup) => {
      ...
      if (llExitIntent) {
        ...
        window.addEventListener("mouseout", (event) => {
          ...
          if (llExitModalShown || event.relatedTarget || event.clientY > 0)
            return;
          popup.classList.add("visible");
          countdowns.forEach((countdown) => {
            triggerCountdown(countdown);
...
ll_links_for_replace.forEach((el) => {
  ...
  const shouldSkip = SKIP_PROTOCOLS.some((protocol) =>
    hrefAttr.startsWith(protocol),
  );
...
/* DAY PICKER SCRIPT */
/* DAY PICKER SCRIPT */
const ll_day_pickers = document.querySelectorAll(
...
ll_day_pickers.forEach((el) => {
  ...
  if (dateFormat === "year") dateSpan.innerText = new Date().getFullYear();
    ...
    dateSpan.innerText = setdate.toLocaleString("en-us", {
...
/* SPINNER SCRIPT */
/* SPINNER SCRIPT */
// const ll_spiners = document.querySelectorAll('#spinner');

// let spin = 0;
// // let rotation = 0;

// ll_spiners.forEach((spinner) => {
//   const ll_spinner_img = spinner.querySelector('#spinBG');
...
//     const spinDeg = spinner.dataset.spinDeg || 150;
//     const spinnerLossDeg = spinner.dataset.spinnerLossDeg || 30;
...
//     if (spin === 2) {
//       ll_spinner_img.style.transform = `translate(-50%, -50%) rotate(${+spinDeg + 720}deg)`;
...
//     if (spin === 3) {
//       ll_spinner_img.style.transform = `translate(-50%, -50%) rotate(${+spinDeg + 1440}deg)`;
//       spinner.classList.add('ll-spinner--spin');
...
//     // if (count === 1) {
//     //   rotation = 1440 + 30;
//     //   ll_spinner_img.style.transform = `translate(-50%, -50%) rotate(${rotation}deg)`;
//     //   setTimeout(() => openPopupById('modal02'), 6000);
//     // } else if (count === 2) {
//     //   rotation += 1440 + 120;
//     //   ll_spinner_img.style.transform = `translate(-50%, -50%) rotate(${rotation}deg)`;
//     //   setTimeout(() => openPopupById('modal03'), 6000);
//     // } else if (count === 3) {
//     //   rotation += 1440 + 240;
//     //   ll_spinner_img.style.transform = `translate(-50%, -50%) rotate(${rotation}deg)`;
//     //   setTimeout(() => openPopupById('modal04'), 6000);
//     // }
//   });
// });
...
// function ll_show_popup(modalId) {
//   const allPopups = document.querySelectorAll('[ll-modal-id]');
//   allPopups.forEach((p) => {
//     p.classList.remove('visible');

//     const target = document.querySelector(`[ll-modal-id="${modalId}]`);
...
// document.getElementById('modalButtonFirst').addEventListener('click', () => {
//   closeModal();
// });
...
//   const spinDeg = 150;
//   const spinnerLossDeg = 30;

//   if (spin === 1) {
//     spinBG.className = 'spinAround';
//     void spinBG.offsetWidth;
//     spinner.classList.add('ll-spinner--spin');
...
window.addEventListener('load', () => {
  spinBG.style.animation = 'none';
  spinBG.style.transform = 'rotate(0deg)';
  showModal('modal01');
});
...
spinner.addEventListener('click', () => {
  ...
  let targetDeg = 0;
  const normalized = currentDeg % 360;

  if (spin === 1) targetDeg = 315;
  if (spin === 2) targetDeg = 360;
  if (spin === 3) targetDeg = 90;

  const addDeg = 1440 + targetDeg - normalized;
  currentDeg += addDeg;

  spinBG.style.transition = 'transform 6s ease';
  spinBG.style.transform = `rotate(${currentDeg}deg)`;

  setTimeout(() => {
    spinner.classList.remove('ll-spinner--spin');

    console.log('SHOW MODAL FOR SPIN:', spin);

    if (spin === 1) showModal('modal02');
    if (spin === 2) showModal('modal03');
    if (spin === 3) showModal('modal04');
...
/* VIDEO SCRIPT */
/* VIDEO SCRIPT */
const ll_video_elements = document.querySelectorAll(
    ...
    llForms.forEach((llForm) => {
      ...
      if (googleRecaptchaV2SiteKey && submitButton) {
        if (typeof window.grecaptcha !== "undefined") {
          ...
          initializeGoogleRecaptchaForForm(
            llForm,
            submitButton,
            googleRecaptchaV2SiteKey,
            googleRecaptchaV2Size,
          );
        } else {
          console.warn(
            "Google reCAPTCHA script not found in page. Please include the reCAPTCHA script.",
          );
        }
      }
      ...
      if (cfTurnstileSiteKey && submitButton) {
        if (typeof window.turnstile !== "undefined") {
          createFormCaptchaCallback(llForm.id);
          initializeCloudflareTurnstileForForm(
            llForm,
            submitButton,
            cfTurnstileSiteKey,
          );
          ...
          console.warn(
            "Cloudflare Turnstile script not found in page. Please include the Turnstile script.",
          );
        }
      }
    });
 ...
 */
function initializeGoogleRecaptchaForForm(
  form,
  submitButton,
  googleRecaptchaV2SiteKey,
  googleRecaptchaV2Size,
 ...
 */
function initializeCloudflareTurnstileForForm(
  form,
  submitButton,
  cfTurnstileSiteKey,
) {
...
(function () {
  ...
  nav.addEventListener("click", (e) => {
    if (
      e.target.classList.contains("ll-nav__overlay") ||
      e.target.closest(
        '[data-gjs-type="nav-links"] a, [data-gjs-type="nav-links"] button',
      )
    ) {
      closeMenu();
    }
...
(function () {
  ...
  function dismissBar(bar) {
    ...
    if (stayDismissed && cookieName && !isPreview())
  ...
  window.llStickyBars = {
    show: function (barId) {
      resolveBars(barId).forEach(showBar);
    },
    hide: function (barId) {
      resolveBars(barId).forEach(hideBar);
    },
    dismiss: function (barId) {
      resolveBars(barId).forEach(dismissBar);
    },
    toggle: function (barId) {
      resolveBars(barId).forEach(toggleBar);
    },
    get: function (barId) {
  ...
  const initAnimations = () => {
    ...
    elements.forEach((el) => {
      ...
      const hasEntrance = animations.some(
        (anim) =>
          isEntrance(anim.preset) &&
          (anim.trigger === "scroll" || anim.trigger === "load"),
      );
      if (hasEntrance) {
              ...
              if (data) {
                setTimeout(() => {
                  data.anims.forEach((anim) =>
                    playAnimation(entry.target, anim),
                  );
                }, SCROLL_PLAY_DELAY);
              }
